...
class MITStates(BaseDataset):
  ...
  def __init__(self, path, split='train', transform=None):
    ...
    self.filepath2ix_in_npy = json.load(open("filepath2ix_in_npy.txt", "r"))
    # self.wv = KeyedVectors.load('wordvectors-300.kv', mmap='r')
    self.regions_data = np.load('../data/regions/train_ims.npy', mmap_mode='r')
    ...
    for f in listdir(path + '/images'):
      ...
      for file_path in listdir(path + '/images/' + f):
        ...
        s = path + 'images/' + f + '/' + file_path
        s = '/home/ubuntu' + s[2:]
        if s in self.filepath2ix_in_npy:
            self.imgs += [{
            ...
            print(s, 'not in filepath2ix_in_npy')
  ...
  def __getitem__(self, idx):
    ...
    return {
        ...
        'source_regions_data': self.get_regions(idx),
        'noun' : self.imgs[idx]['noun'],
        # 'w2v_data' : self.get_w2v_features(idx, target_idx),
        'source_caption': self.imgs[idx]['captions'][0],
  ...
  def get_regions(self, idx):
    try:
        regions = self.filepath2ix_in_npy['/home/ubuntu' + self.imgs[idx]['file_path'][2:]] # hard coded abs path
        return self.regions_data[regions]
    except KeyError:
        print("key", '/home/ubuntu' + self.imgs[idx]['file_path'][2:], "doesn't exist")
        pass
    # return self.regions_data[regions]

  def get_w2v_features(self, idx, target_idx):
    target_adj = self.imgs[target_idx]['adj']
    noun = self.imgs[idx]['noun']
    source_adj = self.imgs[idx]['adj']

    return self.wv[source_adj], self.wv[noun], self.wv[target_adj]
