...
def create_model_and_optimizer(opt, texts):
    ...
    if opt.model == 'imgonly':
        ...
        if opt.use_pretrained:
            ...
            regions_model_checkpoint = torch.load(
                '../logs/mitstates/Feb22_21-33-49_ip-172-31-38-215mitstates_tirg_evolved_regions_05Drop/latest_checkpoint.pth')
            full_model_checkpoint = torch.load(
                '../logs/mitstates/Feb15_22-52-59_ip-172-31-38-215mitstates_tirg_original_text_model/latest_checkpoint.pth')
    ...
    while it < opt.num_iters:
        ...
        def training_1_iter(data):
            ...
            if opt.dataset == 'css3d':
                ...
                for obj in objects:
                    ...
                    for desc in obj:
                        if desc['shape']:
                            obj_desc.append(
                                desc['pos_str'] + ' ' + desc['size'] + ' ' + desc['color'] + ' ' + desc['shape'])
            ...
            # tirg evolved
                ...
                loss_value, img2, encoded_imgs, img1, repr_to_compare_with_source, repr_to_compare_with_mods, text_features = model.compute_loss_with_extra_data(
                    img1,
                    mods,
                    img2,
                    extra_data,
            ...
            positive = L2_loss(repr_to_compare_with_source, img1)
            positive_text = L2_loss(repr_to_compare_with_mods, text_features)
            ...
            losses += [("L2_loss", 0.1, cos_loss)]
            losses += [("L2_loss_text", 0.1, cos_loss_text)]
            total_loss = sum([
